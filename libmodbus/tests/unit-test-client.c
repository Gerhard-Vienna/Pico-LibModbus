...
// libmodbus/src/modbus-private.h:#define _RESPONSE_TIMEOUT 500000
// libmodbus/src/modbus-private.h:#define _BYTE_TIMEOUT     500000
//
// libmodbus/src/modbus-pico-tcp.h:#define CLIENT_TIMEOUT  (60 * 1000)
//
// libmodbus-3.1.11/src/modbus-private.h:#define _RESPONSE_TIMEOUT 500000
// libmodbus-3.1.11/src/modbus-private.h:#define _BYTE_TIMEOUT     500000
...
int main(int argc, char *argv[])
{
    ...
    usleep(1600000);
    modbus_flush(ctx);
    modbus_set_response_timeout(ctx, 1, 600000);
    rc = modbus_read_registers(
        ctx, UT_REGISTERS_ADDRESS_SLEEP_500_MS, 1, tab_rp_registers);
    printf("6/6 Adequate response timeout (1+0.6s > 1+0.5s): ");
#endif
    ASSERT_TRUE(rc == 1, "");
    ...
    if (use_backend == TCP) {
        ...
        ASSERT_TRUE(rc == 1, "");
#else
        /* Timeout of 20ms between bytes */
        modbus_set_byte_timeout(ctx, 0, 50 *1000);
        rc = modbus_read_registers(
            ctx, UT_REGISTERS_ADDRESS_BYTE_SLEEP_5_MS, 1, tab_rp_registers);
        printf("2/2 Adapted byte timeout (50ms > 5ms): "); // (???)
        ASSERT_TRUE(rc == 1, "");
#endif
    }
