 ...
 *
 * This file has been modified for use with a modbus device running on a RP2040.
 *
 * All modifications are within PICO_W ifdef's.
 *
...
static void _sleep_response_timeout(modbus_t *ctx)
{
    ...
    timeout_ms =  ctx->response_timeout.tv_sec * 1000;
    timeout_ms += ctx->response_timeout.tv_usec / 1000;

    busy_wait_ms(timeout_ms);
#endif
}
...
//                     printf("TEST send_msg connect\n");
                    modbus_close(ctx);
                    _sleep_response_timeout(ctx);
                    modbus_connect(ctx);
                } else {
//                     printf("TEST send_msg flush\n");
                    _sleep_response_timeout(ctx);
                    modbus_flush(ctx);
                }
                errno = saved_errno;
    ...
    while (length_to_read != 0) {
        ...
        if (rc == -1) {
            ...
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK) {
                ...
                if (errno == ETIMEDOUT) {
//                     printf("TEST _modbus_receive_msg flush\n");
                    _sleep_response_timeout(ctx);
                    modbus_flush(ctx);
                } else if (errno == EBADF) {
//                     printf("TEST _modbus_receive_msg connect\n");
                    modbus_close(ctx);
                    modbus_connect(ctx);
                }
                errno = saved_errno;
                 ...
                 modbus_mapping_t *mb_mapping)
{
    ...
    case MODBUS_FC_READ_EXCEPTION_STATUS:
#ifdef PICO_W
        ctx->backend->mapping_unlock(ctx);
#endif
        if (ctx->debug) {
